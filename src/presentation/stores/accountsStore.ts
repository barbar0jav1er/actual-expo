...
interface AccountsActions {
  ...
  createAccount: (name: string, offbudget?: boolean, balance?: number) => Promise<void>
...
export const useAccountsStore = create<AccountsStoreInternal>((set, get) => ({
  ...
  createAccount: async (name: string, offbudget = false, balance = 0) => {
    const { _createAccount, fetchAccounts } = get()
    if (!_createAccount) return
    set({ isLoading: true, error: null })
    try {
      await _createAccount.execute({ name, offbudget, balance })
      await fetchAccounts()
